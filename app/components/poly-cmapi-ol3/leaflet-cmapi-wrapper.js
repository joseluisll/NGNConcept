...
/**
  *  This module HAS CODE THAT INVOKES FUNCTIONS FROM A LEAFLET MAP
  *  This module if not connected to the CMAJS in any way!
  */

var leaflet_cmapi_wrapper = {
    ...
    overlay: { 
        ...
        create: function (channel, payload) { 
            ...
            if (payload.hasOwnProperty('overlayId')) { 
                ...
                if (leaflet_cmapi_wrapper.overlays.hasOwnProperty(id)) {
                    console.log('overlay with id ' + id + 'already exists'); 
                    return; 
                } 
            } else { 
                id = leaflet_cmapi_wrapper.createGuid();
            } 
            ...
            if (payload.hasOwnProperty('parentId')) { 
                ...
                if (!leaflet_cmapi_wrapper.overlays.hasOwnProperty(parentId)) {
                    leaflet_cmapi_wrapper.overlay.create("map.overlay.create", {
                        overlayId: parentId 
                    }); 
                } 
            } else { 
            ...
            newOverlay.addTo(leaflet_map);
            leaflet_map.getPanes()[id] = ""; //TODO: shoudl remove features before doing this

            // add to overlay data store 
            leaflet_cmapi_wrapper.overlays[id] = {
                obj: newOverlay, 
                name: name, 
                parent: parentId, 
                children: {}
            ...
            console.log(leaflet_cmapi_wrapper.overlays[id]);
        }, 
        ...
        remove: function (channel, payload) { 
            ...
            overlay = leaflet_cmapi_wrapper.overlays[id];
            overlayObj = overlay.obj; 
  
            // delete from map; also deletes all child overlays 
            leaflet_map.removeLayer(overlayObj);
  
            // delete all children in storage 
            for (var key in leaflet_cmapi_wrapper.feature.features) {
                var feature = leaflet_cmapi_wrapper.feature.features[key].overlayId;
                if (feature === id) {
                    delete leaflet_cmapi_wrapper.feature.features[key]
                }
                console.log('child overlay with id ' + id + ' removed'); 
            } 
            // and then delete the overlay in storage 
            delete leaflet_cmapi_wrapper.overlays[id];
        ...
        hide: function (channel, payload) { 
            ...
            if (!leaflet_cmapi_wrapper.overlays.hasOwnProperty(payload.overlayId)) {
                console.log("Overlay doesn't exist to hide: " + payload.overlayId);
                return;
            }

            // Hide map pane (features with current overlayId)
            $(leaflet_map.getPane(payload.overlayId)).hide();
        ...
        show: function (channel, payload) { 
            ...
            $(leaflet_map.getPane(payload.overlayId)).show();
        ...
        update: function (channel, payload) { 
            ...
            if (!payload.hasOwnProperty("overlayId")) {
                ...
                var overlay = leaflet_cmapi_wrapper.overlays[payload.overlayId];
                ...
                if (payload.hasOwnProperty("parentId")) {
                    ...
                    // create parent if it does not exist 
                    if (!leaflet_cmapi_wrapper.overlays.hasOwnProperty(payload.parentId)) {
                        leaflet_cmapi_wrapper.overlay.create("map.overlay.create", {
    ...
    feature: { 
        ...
        plotFeature: function (channel, payload) { 
            ...
            if (_.isEmpty(payload.overlayId) ||
                _.isEmpty(leaflet_cmapi_wrapper.overlays[payload.overlayId])) {
                overlayId = leaflet_cmapi_wrapper.createGuid();
                leaflet_cmapi_wrapper.overlay.create('channel', {overlayId: overlayId});
            } else { // Overlay exists.
            ...
            if (payload.format === "geojson") {
                ...
                leaflet_map.addLayer(obj);
                leaflet_cmapi_wrapper.feature.features[payload.featureId] =
                    {marker: obj,
                        overlayId: overlayId};
        ...
        unplotFeature: function (channel, payload) { 
            ...
            leaflet_map.removeLay(leaflet_cmapi_wrapper.feature.features[payload.featureId].marker);
            delete leaflet_cmapi_wrapper.feature.features[payload.featureId];
        ...
        hideFeature: function (channel, payload) { 
            ...
            leaflet_cmapi_wrapper.feature.features[payload.featureId].marker.setOpacity(0);
        ...
        showFeature: function (channel, payload) { 
            ...
            //TOD: Add error checking
            leaflet_cmapi_wrapper.feature.features[payload.featureId].marker.setOpacity(1);
            // marker.setOpacity(1 or 0);
        }, 
    ...
    view: {
        ...
        zoom: function (channel, payload) {
            ...
            leaflet_map.setZoom(payload.range);
        },
        ...
        centerOnOverlay: function (channel, payload) {
            ...
            if (typeof payload.overlayId === 'string'){
                ...
                var bounds = leaflet_cmapi_wrapper.overlays[payload.overlay].obj.getBounds();
            } else {
                //Find the default overlay and use its center
                var bounds = leaflet_cmapi_wrapper.overlays['default'].obj.getBounds();
            }
        ...
        centerOnLocation: function (channel, payload) {
            ...
            leaflet_map.setView([payload.location.lat,payload.location.lon],range);
        },
        ...
        centerOnBounds: function (channel, payload) {
            ...
            leaflet_map.fitBounds(L.latLngBounds(sw,ne));
        },
        ...
        mapClicked: function () {
            ...
            leaflet_map.fireEvent('click', eventstuff);
        }
        ...
        respondWithMapViewStatus: function () {
            ...
            */
            return ({/* requester: (optional), */
                    "bounds": {"southWest": {"lat": leaflet_map.getPixelOrigin().y+leaflet_map.getSize().y, "lon": leaflet_map.getPixelOrigin().x},
                               "northEast": {"lat": leaflet_map.getPixelOrigin().y, "lon": leaflet_map.getPixelOrigin().x+leaflet_map.getSize().x}},
                    "center": {"lat": leaflet_map.getCenter().lat, "lon": leaflet_map.getCenter().lng},
                    "range": leaflet_map.getZoom() });
        },
